...
case "$uname_output" in
  "Linux")                  my_platform="linux";;
  "Darwin")                 my_platform="darwin";;
  "Solaris"|"SunOS")        my_platform="solaris";;
  "BSD"|"DragonFly")        my_platform="bsd";;
  "Haiku")                  my_platform="beos";;
  "MINGW"|"mingw"|"MinGW")  my_platform="mingw";;
  "CYGWIN"|"cygwin")        my_platform="cygwin";;
  *)
...
# Determine CPU info
#
if test -e "/proc/cpuinfo"; then # Linux
  num_cpus=$(grep -c processor /proc/cpuinfo)
  cpu_vendor=$(grep "Vendor ID:" /proc/cpuinfo | grep -o "\w\+$" | head -1)
elif command -v lscpu > /dev/null; then # Linux Alternative
  num_cpus=$(lscpu | grep -i "CPU(s):" | awk '{print $2}')
  cpu_vendor=$(lscpu | grep "Vendor ID:" | grep -o "\w\+$" | head -1)
elif command -v psrinfo > /dev/null; then # Solaris
  num_cpus="$(psrinfo | wc -l)"
  # TODO: cpu_vendor
elif test -n "$(which sysinfo 2> /dev/null)"; then # Haiku/BeOS
  num_cpus="$(sysinfo | grep -c -i "CPU #[0-9]*:")"
  # TODO: cpu_vendor
elif test -n "$(which sysctl 2> /dev/null)"; then # BSD
  # shellcheck disable=SC2196
  num_cpus="$(sysctl -a 2> /dev/null | egrep -i 'hw.ncpu' | awk '{print $2}')"
  # TODO: cpu_vendor
fi

if test -n "$(which arch 2> /dev/null)"; then # Linux
  my_arch="$(arch)"
elif test -n "$(which lscpu 2> /dev/null)"; then # Linux Alternative
  my_arch="$(lscpu | grep -i "Architecture" | awk '{print $2}')"
elif test -n "$(which sysinfo 2> /dev/null)"; then # Haiku/BeOS
  my_arch="$(sysinfo | grep -o "kernel_\\w*")"
else # BSD/Solaris
  my_arch="$(uname -p 2> /dev/null)"
  test -n "$my_arch" || my_arch="$(uname -m 2> /dev/null)"
fi

if test -n "$(echo "$my_arch" | grep -i "sparc\\|sun4u")" ; then
  my_arch_family="sparc"
elif test -n "$(echo "$my_arch" | grep -i "^ppc\\|powerpc")" ; then
  my_arch_family="powerpc"
elif test "$my_arch" =~ mips ; then
  my_arch_family="mips"
elif test "$my_arch" =~ s390 ; then
  my_arch_family="s390x"
elif test -n "$(echo "$my_arch" | grep -i "ia64\\|itanium")" ; then
  my_arch_family="itanium"
elif test "$my_arch" =~ arm64 ; then
  my_arch_family="arm64"
elif test "$my_arch" =~ arm ; then
  my_arch_family="arm"
elif test "$my_arch" =~ 64 ; then
  my_arch_family="x86_64"
elif test -n "$(echo "$my_arch" | grep -i "\\(i[3-6]\\?\\|x\\)86\\|ia32")"; then
  my_arch_family="i386"
fi
...
# Setup Build Env
#
if test -z "$(echo "$PATH" | grep "/usr/local/bin")" ; then
  export PATH=/usr/local/sbin:/usr/local/bin:$PATH
fi
if test "$PATH" =~ "rvm/bin" ; then
...
if test "$my_platform" == "linux" ; then
  ...
  if test -z "$(which lsb_release 2> /dev/null)" ; then
    if test -n "$(which apt-get 2> /dev/null)" ; then    # Debian/Ubuntu/Linux Mint/PCLinuxOS
      sudo apt-get install -y lsb
    elif test -n "$(which up2date 2> /dev/null)" ; then  # RHEL/Oracle
      sudo up2date -i lsb
    elif test -n "$(which dnf 2> /dev/null)" ; then      # CentOS/Fedora/RHEL/Oracle
      sudo dnf install -y lsb
    elif test -n "$(which yum 2> /dev/null)" ; then      # Old CentOS/Fedora/RHEL/Oracle
      sudo yum install -y lsb
    elif test -n "$(which zypper 2> /dev/null)" ; then   # OpenSUSE/SLES
      sudo zypper --non-interactive install lsb
    elif test -n "$(which pacman 2> /dev/null)" ; then   # ArchLinux
      sudo pacman -S --noconfirm lsb-release
    elif test -n "$(which urpmi 2> /dev/null)" ; then    # Mandriva/Mageia
      sudo urpmi --auto lsb-release
    elif test -n "$(which emerge 2> /dev/null)" ; then   # Gentoo
      sudo emerge lsb
    elif test -n "$(which slackpkg 2> /dev/null)" ; then # Slackware
  ...
  if test -n "$(echo "$lsb_release_output" | grep -i "debian")" ; then
    my_distro="debian"
    my_pkg_fmt="deb"
    my_pkg_mgr="apt-get"
    my_install="apt-get install -y"
    my_local_install="dpkg -i"
    if test "$my_arch_family" == "x86_64" ; then my_pkg_arch="amd64"; fi
  elif test -n "$(echo "$lsb_release_output" | grep -i "ubuntu")" ; then
    my_distro="ubuntu"
    my_pkg_fmt="deb"
    my_pkg_mgr="apt-get"
    my_install="apt-get install -y"
    my_local_install="dpkg -i"
    if test "$my_arch_family" == "x86_64" ; then my_pkg_arch="amd64"; fi
  elif test -n "$(echo "$lsb_release_output" | grep -i "mint")" ; then
    my_distro="mint"
    my_pkg_fmt="deb"
    my_pkg_mgr="apt-get"
    my_install="apt-get install -y"
    my_local_install="dpkg -i"
    if test "$my_arch_family" == "x86_64" ; then my_pkg_arch="amd64"; fi
  elif test -n "$(echo "$lsb_release_output" | grep -i "centos")" ; then
    my_distro="centos"
    my_pkg_fmt="rpm"
    my_pkg_mgr="yum"
    my_install="yum install -y"
    my_local_install="yum localinstall -y"
  elif test -n "$(echo "$lsb_release_output" | grep -i "fedora")" ; then
    my_distro="fedora"
    my_pkg_fmt="rpm"
    if test "$my_major_release" -ge "18"; then
      my_pkg_mgr="dnf"
  ...
  elif test -n "$(echo "$lsb_release_output" | grep -i "redhat\|rhel")" ; then
    my_distro="rhel"
    my_pkg_fmt="rpm"
    my_pkg_mgr="up2date"
    ...
    my_install="up2date -i"
    my_local_install="rpm -Uvh"
  elif test -n "$(echo "$lsb_release_output" | grep -i "open\s*suse")" ; then
    my_distro="opensuse"
    my_pkg_fmt="rpm"
    my_pkg_mgr="zypper"
    my_install="zypper --non-interactive install"
    my_local_install="rpm -Uvh"
  elif test -n "$(echo "$lsb_release_output" | grep -i "suse" | grep -i "enterprise")" ; then
    my_distro="sles"
    my_pkg_fmt="rpm"
    my_pkg_mgr="zypper"
    my_install="zypper --non-interactive install"
    my_local_install="rpm -Uvh"
  elif test -n "$(echo "$lsb_release_output" | grep -i "archlinux")" ; then
    my_distro="arch"
    ...
    my_major_release="$(echo "$my_nickname" | grep -o "[0-9]\+" | head -1)"
    if test "$my_major_release" -lt "13" ; then
      my_pkg_fmt="tgz"
    else
      my_pkg_fmt="txz"
    fi
  ...
  elif test -n "$(echo "$lsb_release_output" | grep -i "mandriva")" ; then
    my_distro="mandriva"
    my_pkg_fmt="rpm"
    my_pkg_mgr="urpmi"
    my_install="urpmi --auto "
    my_local_install="rpm -Uvh"
  elif test -n "$(echo "$lsb_release_output" | grep -i "mageia")" ; then
    my_distro="mageia"
    my_pkg_fmt="rpm"
    my_pkg_mgr="urpmi"
    my_install="urpmi --auto "
    my_local_install="rpm -Uvh"
  elif test -n "$(echo "$lsb_release_output" | grep -i "gentoo")" ; then
    my_distro="gentoo"
    my_pkg_fmt="tgz"
    my_pkg_mgr="emerge"
    my_install="emerge"
  ...
  # TODO: Add Clear Linux w/ pkg manager "apk" to the list
  else
...
elif test "$my_platform" == "darwin" ; then

  my_distro="Mac OSX"
  my_install=""

  if test -n "$(which brew 2> /dev/null)" ; then # Homebrew
    my_method="install"
    my_install="brew install"
  elif test -n "$(which port 2> /dev/null)" ; then # MacPorts
    my_method="install"
  ...
  case "$my_major_release" in
    ...
    "10.15") my_nickname="Catalina";;
    "11.0") my_nickname="Big Sur";;
    *)
...
elif test "$my_platform" == "solaris" ; then

  my_major_release="$(uname -r | grep -o "[0-9]\+" | head -2 | tail -1)"
  if test -n "$(uname -a | grep -i "open\s*solaris")" ; then
    my_distro="OpenSolaris"
    my_nickname="$my_distro $(grep -o "OpenSolaris [a-zA-Z0-9.]\\+" /etc/release)"
  else
    my_distro="Solaris"
...
elif test "$my_platform" == "bsd" ; then
...
elif test "$my_platform" == "beos" ; then
...
elif test "$my_platform" == "mingw" || "$my_platform" == "cygwin" ; then

  my_distro="Windows"
  my_install=""
  my_host_platform="windows"

  if test "$my_platform" == "mingw" ; then # MinGW
...
#    my_install="mingw-get.exe install"
    my_method="build"
    my_install=""
  elif test "$my_platform" == "cygwin" ; then # Cygwin
    my_method="install"
...
export cpu_vendor
export my_distro
export my_major_release
export my_pkg_fmt
export my_pkg_mgr
export my_pkg_arch
export my_nickname
export my_host_platform
export my_method
export my_install
export my_local_install
